...
public class HwAutocompleteManager: NSObject, InputPlugin, UITextViewDelegate, UITableViewDelegate, UITableViewDataSource {
    weak var dataModel: AutocompleteDataModel?
    // MARK: - Properties [Public]
    
    /// A protocol that passes data to the `AutocompleteManager`
    open weak var dataSource: HwAutocompleteManagerDataSource?
    
    /// A protocol that more precisely defines `AutocompleteManager` logic
    open weak var delegate: HwAutocompleteManagerDelegate?
    ...
    private(set) public var currentSession: HwAutocompleteSession?
    ...
    /// from the `dataSource`, based on the `HwAutocompleteSession`.
    /// The default function requires the `AutocompleteCompletion.text`
    /// string contains the `HwAutocompleteSession.filter`
    /// string ignoring case
    open var filterBlock: (HwAutocompleteSession, AutocompleteCompletion) -> (Bool) = { session, completion in completion.text.lowercased().contains(session.filter.lowercased())
    }
    ...
    open func reloadData() {
        ...
        let wordWithoutPrefix = (result.word as NSString).substring(from: result.prefix.utf16.count)
        guard let session = HwAutocompleteSession(prefix: result.prefix, range: result.range, filter: wordWithoutPrefix) else { return }
    ...
    open func autocomplete(with session: HwAutocompleteSession) {
    ...
    ///
    /// - Parameter session: The `HwAutocompleteSession` to form an `NSMutableAttributedString` with
    /// - Returns: An `NSMutableAttributedString`
    open func attributedText(matching session: HwAutocompleteSession,
                             fontSize: CGFloat = 15,
    ...
    ///   - sesstion: The 'HwAutocompleteSession'
    ///   - range: The 'NSRange' to insert over
    private func insertAutocomplete(_ autocomplete: String, at session: HwAutocompleteSession, for range: NSRange) {
...
//        print("Inserted string \(newAttributedText)")
    }
    
    /// Initializes a session with a new `HwAutocompleteSession` object
    ///
    /// - Parameters:
    ///   - session: The session to register
    private func registerCurrentSession(to session: HwAutocompleteSession) {
    ...
    private func layoutIfNeeded() {
        ...
        if let session = currentSession {
            dataModel.items =  currentAutocompleteOptions.map{ completion in
                session.completion = completion
                let text = attributedText(matching: session, fontSize: 15)
    //            print("Attributed text \(text)")
                return AutocompleteItem(content: text, completion: { [weak self] in
                    if let self = self {
                        guard let session = self.currentSession else {
                            print("HwAutocompleteManager: currentSession is nil")
                            return
                            
                        }
                        print("HwAutocompleteManager: tapped")
                        session.completion = completion
                        self.autocomplete(with: session)
                    } else {
                        print("HwAutocompleteManager: self is nil")
                    }
    ...
    open func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        guard let session = currentSession else { fatalError("Attempted to render a cell for a nil `HwAutocompleteSession`") }
